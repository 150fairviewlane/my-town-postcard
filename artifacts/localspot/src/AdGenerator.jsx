...
size={36 * fScale} bg={ind.colors.primary} color="#fff" border={`2px solid #fff`} />
  ...
  <div style={{ flex: 1, padding: `${10*fScale}px ${12*fScale}px`, display: "flex", flexDirection: "column", justifyContent: "space-between", background: ind.colors.light, minWidth: 0 }}>
    ...
    {/* Menu: show for all sizes */}
    {true && (
      <div style={{ display: "flex", flexDirection: "column", gap: 3*fScale, margin: `${5*fScale}px 0` }}>
        {getActiveItems(data.menuItems, ind.menu).slice(0, isXL ? 3 : isL ? 3 : isM ? 2 : 1).map((item, i) => (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {data.address && (
            <EditableText value={data.address.split(",")[0]} onChange={edit("address")}
              style={{ fontSize: Math.max(11, 10*fScale), color: "#555", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }} />
          )}
          {data.phone && (
            <EditableText value={data.phone} onChange={edit("phone")}
...
borderRadius: 4, fontFamily: "sans-serif", whiteSpace: "nowrap",
  ...
  <div style={{ flex: 1, padding: `${4*fScale}px ${10*fScale}px ${5*fScale}px`, display: "flex", flexDirection: "column", justifyContent: "space-between", minHeight: 0 }}>
    ...
    {/* Menu -- all sizes, fewer items for S/M */}
    {true && (
      <div style={{ display: "flex", flexWrap: "wrap", gap: `${3*fScale}px ${10*fScale}px`, margin: `${4*fScale}px 0` }}>
        {getActiveItems(data.menuItems, ind.menu).slice(0, isXL ? 4 : isL ? 4 : isM ? 3 : 2).map((item, i) => (
...
background: `linear-gradient(180deg, ${ind.colors.dark}50 0%, ${ind.colors.dark}cc 100%)`
  ...
  <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: `${6*fScale}px ${10*fScale}px ${8*fScale}px`, display: "flex", flexDirection: "column", gap: 4*fScale, zIndex: 3 }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontFamily: "sans-serif" }}>
      <div>
        {data.address && <EditableText value={data.address.split(",")[0]} onChange={edit("address")} style={{ color: "rgba(255,255,255,0.85)", fontSize: Math.max(10, 9*fScale), display: "block", whiteSpace: "nowrap" }} />}
        {isS && data.phone && <EditableText value={data.phone} onChange={edit("phone")} style={{ color: "rgba(255,255,255,0.95)", fontSize: Math.max(11, 10*fScale), fontWeight: 700, display: "block", whiteSpace: "nowrap" }} />}
      </div>
      {hasQR(data) && !isS && (
        <InlineQRCode
          website={normalizeWebsite(data.website)}
...
background: leftBg, fontFamily: "sans-serif",
  ...
  <div style={{
    ...
    <div>
      ...
      {/* Service checklist -- show on L/XL; show 2 on M */}
      {!isS && (
        <div style={{ display: "flex", flexDirection: "column", gap: 3*fScale }}>
          {getActiveItems(data.menuItems, ind.menu).slice(0, isXL ? 5 : isL ? 4 : 2).map((item, i) => (
...
if (!menuItems || menuItems.length === 0) return fallback || [];
return menuItems
