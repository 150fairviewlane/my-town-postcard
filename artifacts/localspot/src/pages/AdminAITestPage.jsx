...
const HDR_LINK = { fontSize: 13, fontWeight: 700, color: "#374151", background: "#fff", border: "1px solid #d1d5db", borderRadius: 8, padding: "7px 12px", textDecoration: "none", cursor: "pointer" };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: "sans-serif" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "12px 28px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
        <button onClick={() => navigate("/admin")} style={HDR_LINK}>← Admin</button>
        <div>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 20px", display: "grid", gridTemplateColumns: "340px 1fr", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={CARD}>
            ...
            >
              ...
              {imageData ? (
                <div>
                  <img src={imageData} alt="uploaded" style={{ width: "100%", borderRadius: 7, display: "block", maxHeight: 220, objectFit: "contain" }} />
          ...
          >
            {running ? `Running ${selected.length} model${selected.length > 1 ? "s" : ""} in parallel…` : `▶ Run ${selected.length} Model${selected.length !== 1 ? "s" : ""}`}
          </button>
          {running && (
            <div style={{ fontSize: 12, color: "#6b7280", textAlign: "center", lineHeight: 1.6 }}>
              All models run in parallel. Expect ~60 seconds per model.<br />
              Results appear when all finish.
            </div>
          )}
          {globalError && (
        ...
        <div>
          ...
          {running && (
            <div style={{ ...CARD, textAlign: "center", padding: 56 }}>
              <div style={{ width: 40, height: 40, border: "4px solid #e5e7eb", borderTopColor: "#991b1b", borderRadius: "50%", animation: "spin .8s linear infinite", margin: "0 auto 16px" }} />
          ...
          {results.length > 0 && (
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(results.length, 3)}, 1fr)`, gap: 16 }}>
                {results.map((r) => {
                  const meta = ACTIVE_MODELS.find((m) => m.id === r.model);
                  return (
                    <div key={r.model} style={{ ...CARD, padding: 0, overflow: "hidden" }}>
                      <div style={{ padding: "12px 14px", borderBottom: "1px solid #f3f4f6" }}>
                        ...
                        <div style={{ fontSize: 11, color: "#9ca3af", display: "flex", gap: 8 }}>
                          <span>{fmtTime(r.timeTaken)}</span>
                          {r.error && <span style={{ color: "#991b1b" }}>· failed</span>}
                          {!r.error && <span style={{ color: "#15803d" }}>· success</span>}
                        </div>
                      </div>

                      {r.error ? (
                        <div style={{ padding: 16, background: "#fef2f2", minHeight: 200, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        ...
                        <div>
                          <img src={r.imageUrl} alt={r.label} style={{ width: "100%", display: "block" }} />
                          <div style={{ padding: "10px 14px", display: "flex", gap: 8 }}>
                            <a
                              href={r.imageUrl}
                              download={`ai-test-${r.model}.png`}
                              style={{ flex: 1, textAlign: "center", padding: "7px", borderRadius: 7, background: "#f3f4f6", color: "#374151", fontSize: 12, fontWeight: 700, textDecoration: "none", border: "1px solid #e5e7eb" }}
                            >
                              ↓ Download
                            </a>
                          </div>
                      ...
                      {meta && (
                        <div style={{ padding: "8px 14px 12px", fontSize: 11, color: "#9ca3af", lineHeight: 1.5, borderTop: "1px solid #f9fafb" }}>
                          {meta.desc}
                        </div>
                      )}
                    </div>
