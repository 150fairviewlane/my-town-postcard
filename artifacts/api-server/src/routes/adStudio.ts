...
.ad-photo-layer{
  position:absolute;
  top:9%;left:26%;right:0;bottom:23%;
  background-size:cover;background-position:center top;
  opacity:0;transition:opacity .6s;
  mix-blend-mode:multiply;
}
.ad-photo-layer.active{opacity:.8}

/* TEXT OVERLAY ZONES */
.ov-bizname{position:absolute;top:.8%;left:29%;right:1%;line-height:.88}
.biz-l1{font-family:'Bebas Neue',sans-serif;font-size:clamp(18px,7.2vw,68px);color:#1C1B1A;letter-spacing:.025em;display:block;text-shadow:1px 1px 0 rgba(255,255,255,.2)}
.biz-l2{font-family:'Pacifico',cursive;font-size:clamp(16px,6vw,58px);color:var(--ac,#C8541A);display:block;line-height:.95;margin-top:.02em}

.ov-tagline{position:absolute;top:19.5%;left:1.5%;width:30%}
.tagline-txt{font-family:'Dancing Script',cursive;font-size:clamp(9px,3.6vw,34px);font-weight:700;color:#1C1B1A;line-height:1.18;display:block}
.tagline-rule{display:block;width:55%;height:2px;background:linear-gradient(90deg,var(--ac,#C8541A),transparent);margin-top:5%;border-radius:1px}

.ov-menu{position:absolute;top:57%;left:9%;right:51%}
.menu-row{display:flex;align-items:baseline;gap:2%;height:7%;margin-bottom:.2%}
.mi-name{font-family:'Montserrat',sans-serif;font-size:clamp(7px,2.1vw,19px);font-weight:800;color:#1C1B1A;text-transform:uppercase;letter-spacing:.04em;white-space:nowrap;flex-shrink:0}
.mi-dots{flex:1;border-bottom:2px dotted rgba(28,27,26,.3);margin-bottom:2px;min-width:5px}
.mi-price{font-family:'Montserrat',sans-serif;font-size:clamp(7px,2.1vw,19px);font-weight:800;color:var(--ac,#C8541A);white-space:nowrap;flex-shrink:0}

.ov-coupon{position:absolute;top:59.5%;left:42%;right:3%;bottom:25%;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:1.5% 3%}
.coupon-amount{font-family:'Bebas Neue',sans-serif;font-size:clamp(14px,5.2vw,50px);color:#fff;line-height:.88;letter-spacing:.02em;display:block}
.coupon-item{font-family:'Pacifico',cursive;font-size:clamp(11px,3.8vw,36px);color:var(--ac,#C8541A);display:block;line-height:1;margin-top:2%}
.coupon-fine{font-family:'DM Sans',sans-serif;font-size:clamp(5px,1.3vw,12px);color:rgba(255,255,255,.55);display:block;margin-top:3%;letter-spacing:.04em}

.ov-phone{position:absolute;top:78.5%;left:11%;right:34%}
.phone-num{font-family:'Bebas Neue',sans-serif;font-size:clamp(12px,4.6vw,44px);color:#fff;letter-spacing:.04em;line-height:1;display:block}
.phone-addr{font-family:'DM Sans',sans-serif;font-size:clamp(5px,1.3vw,12px);color:rgba(255,255,255,.6);display:block;margin-top:1.5%;font-weight:500;letter-spacing:.03em}

.ov-qr-label{position:absolute;bottom:3.5%;right:2%;text-align:center;width:10%}
.qr-url-txt{font-family:'DM Sans',sans-serif;font-size:clamp(4px,1vw,9px);color:rgba(255,255,255,.5);display:block;word-break:break-all;line-height:1.2}

/* EMPTY STATE */
.ad-empty{width:100%;max-width:520px;aspect-ratio:0.8373;background:#1a1a1a;border-radius:12px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:14px;text-align:center;padding:36px;box-shadow:0 16px 60px rgba(0,0,0,.28)}
...
<div class="layout">
  ...
  <div class="fpanel">
    ...
    <div class="fsec">
      ...
      <div style="font-size:10.5px;color:var(--ink-light);margin-top:8px;line-height:1.5">5 additional template styles coming soon. Each uses a different background design with the same business data.</div>
    </div>
    ...
    <div class="fsec">
      ...
      <div class="field">
        ...
        <div class="field-note">The QR code on the template will link to this URL. Leave blank to use phone number as the scan target.</div>
      </div>
    </div>
    ...
    <div class="fsec">
      ...
      <div style="font-size:10.5px;color:var(--ink-light);margin-bottom:8px;line-height:1.5">Format: Item Name $Price &#8212; e.g. &ldquo;Large Pizza $14.99&rdquo;<br>Price is optional &#8212; just enter the item name if no price.</div>
    ...
    <div class="fsec">
      ...
      <div class="field">
        ...
        <input type="text" id="offerAmount" placeholder="e.g. $1 OFF" oninput="renderAd()">
  ...
  <div class="ppanel">
    ...
    <div class="ad-outer">
      ...
      <div class="ad-wrap" id="adWrap">
        ...
        <div class="ad-photo-layer" id="photoLayer"></div>
...
async function loadPhotos(industry, fallback){
  ...
  if(!photos.length){
    grid.innerHTML = '<div class="img-msg">No photos yet for this industry &mdash; upload your own below, or add photos in the Admin Image Library.</div>';
    return;
  }
  grid.innerHTML = '';
  ...
  setPhotoLayer(selectedPhotoUrl);
  renderAd();
}
...
function setPhotoLayer(url){
  var layer = document.getElementById('photoLayer');
  layer.style.backgroundImage = url ? "url('" + url + "')" : 'none';
  layer.classList.toggle('active', !!url);
}
...
function onTemplateChange(){
  var tpl = document.querySelector('input[name="template"]:checked');
  tpl = tpl ? tpl.value : 'rustic';
...
function renderAd(){
  var ac = document.querySelector('input[name="color"]:checked');
  ac = ac ? ac.value : '#C8541A';
  ...
  document.getElementById('ovL2').textContent = document.getElementById('bizLine2').value.trim();
  document.getElementById('ovL2').style.color = ac;
  var tg = document.getElementById('tagline').value.trim();
  document.getElementById('ovTgTxt').textContent = tg;
  ...
  if(amt || item){
    ...
    document.getElementById('ovCpItem').textContent = item;
    document.getElementById('ovCpItem').style.color = ac;
