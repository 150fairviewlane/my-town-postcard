...
export function buildFooterZone(phone: string, address: string, isLandscape = false, sizeKey?: string, templateKey = ""): string {
  ...
  // Bottom-right corner: small bounded warm-gold disc anchored at the image corner.
  // Using a filled circle (not a starburst) because circles have a hard diameter cap —
  // rays can extend arbitrarily, discs cannot. The server erases and recomposites the
  // QR card unconditionally, so the exact shape doesn't matter, but a bounded shape keeps
  // the erasure zone small and the blurred fill visually accurate.
  // "Do NOT draw a QR code" remains so Grok doesn't accidentally produce a scannable pattern.
  const qrSlot =
    `BOTTOM-RIGHT corner: a small solid warm-gold filled circle (disc/dot), ` +
    `diameter no more than 15% of image width. ` +
    `The disc must be tangent to — and not extend beyond — the right and bottom image edges. ` +
    `No rays, no spikes, no starburst, no lens flare. Just a plain filled circle. ` +
  ...
  const outputRequirements = isLandscape && templateKey === "parchment-classic"
    ...
    ? (
      ...
      (d.offer
        ...
        : "") +
      buildFooterZone(d.phone || "", fullAddress, isLandscape, d.sizeKey, templateKey) +
      `STARBURST CORNER: No purple circle/dot-grid, coupon box, cream tile, or leaf sprig in small gold disc corner.\n\n`
    )
    : templateKey === "sage-organic"
    ? (
      ...
      (d.offer
        ...
        : "") +
      buildFooterZone(d.phone || "", fullAddress, isLandscape, d.sizeKey, templateKey) +
      `STARBURST CORNER: No kraft coupon, dark olive badge, or olive wave band in small gold disc corner.\n\n`
    )
    : templateKey === "home-elegance"
    ? (
